...
class bitbucket:
    ...
    def read_repositories_from_file(self):
        ...
        try:
            self.retail_modules_fpt = open(os.path.dirname(__file__) + '\\Data\\' + 'repositories.txt', 'r', encoding = 'utf-8')
            for repo in self.retail_modules_fpt.read().split(self.split_char_for_rf):
                if repo:
                    self.retail_modules.append(repo.strip())
    ...
    def read_projects_from_file(self):
        try:
            self.project_fpt = open(os.path.dirname(__file__) + '\\Data\\' + 'projects.txt', 'r', encoding = 'utf-8')
            for project in self.project_fpt.read().split(self.split_char_for_rf):
                if project:
    ...
    def get_tag(self, project, repo):

        if not(self.list_of_tags):
            ...
            for tag_idx, tag in enumerate(all_tags):
                ...
                if re.search(common_var.dev_tag_re , tag_id):
                    if option_tag_count > 1 and display_tags:
                        print('{} . script\\{}'.format(c + 1, tag_id))
                    c += 1
                    self.recent_tags.append(tag_id)
                    option_tag_count -= 1
    ...
    def close_output_file(self):
        try:
            ...
            print(colorful.red('Err @__Bitbucket__ : '),'No file is opened')
        return
